...
template <class T> auto fib(Const<T> t) {
    static_assert(t >= 0, "");
    return FROZEN(fibImpl(decltype(t){}));
}
...
int main() {
    ...
    static_assert(x1.get() == 3, ""); // constexpr version

    auto x2 = fib(FROZEN(5));
    static_assert(x2.get() == 5, ""); // constexpr version

    constexpr int two = 2;
    auto x3 = fib(FROZEN(two + four()));
    static_assert(x3.get() == 8, ""); // constexpr version
